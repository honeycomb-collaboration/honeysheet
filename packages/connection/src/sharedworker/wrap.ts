import type { MessagePortFromWorkerData, MessagePortToWorkerData } from './data'
...
export function workerWrap(workerUrl: string) {
    ...
    return class Socket implements IConnection {
        ...
        private readonly portMessageHandler: (evt: MessageEvent<MessagePortFromWorkerData>) => void

        constructor(url: string, messageHandler: ConnectionMessageHandler) {
            this.url = url
            this.portMessageHandler = (evt: MessageEvent<MessagePortFromWorkerData>) => {
            ...
            sharedWorker.port.addEventListener('message', this.portMessageHandler)
        ...
        public close(): void {
            sharedWorker.port.removeEventListener('message', this.portMessageHandler)
        }
    }
}
